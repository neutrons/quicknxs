...
class TestDirectBeamFeatureMocked:
    ...
    def test_add_non_direct_beam_run(self):
        ...
        data_handler._nexus_data = mock_nexus

        # Should return 1 (added but not a true direct beam)
        result = data_handler.add_active_to_direct_beam_list()
        ...
        assert len(data_handler.direct_beam_list) == 1
        assert data_handler.direct_beam_list[0].run_number == mock_nexus.run_number
        # It should be a different object (deepcopied)
        assert data_handler.direct_beam_list[0] is not mock_nexus
    ...
    def test_add_duplicate_run(self):
        ...
        result1 = data_handler.add_active_to_direct_beam_list()
        assert result1 == AddToDirectBeamResult.SUCCESS_REFLECTED
        assert len(data_handler.direct_beam_list) == 1

        # Try to add it again
        result2 = data_handler.add_active_to_direct_beam_list()
        assert result2 == AddToDirectBeamResult.ALREADY_IN_LIST
        assert len(data_handler.direct_beam_list) == 1  # Still only one entry
    ...
    def test_mixed_direct_beam_list(self):
        ...
        data_handler._nexus_data = mock_true_db
        result1 = data_handler.add_active_to_direct_beam_list()
        ...
        data_handler._nexus_data = mock_not_db
        result2 = data_handler.add_active_to_direct_beam_list()
        assert result2 == AddToDirectBeamResult.SUCCESS_REFLECTED

        # Both should be in the list
        assert len(data_handler.direct_beam_list) == 2
        assert data_handler.direct_beam_list[0].is_direct_beam() is True
        assert data_handler.direct_beam_list[1].is_direct_beam() is False
    ...
    def test_remove_non_direct_beam_from_list(self):
        ...
        # Remove it
        index = data_handler.remove_active_from_direct_beam_list()
        assert index == 0
        assert len(data_handler.direct_beam_list) == 0
    ...
    def test_update_active_direct_beam_switches_to_direct_beam_copy(self):
        ...
        data_handler._nexus_data = reflected_copy
        data_handler.direct_beam_list = [direct_beam_copy]
        data_handler.set_active_cross_section = mock.Mock(return_value=True)

        data_handler.update_active_direct_beam()

        assert data_handler._nexus_data is direct_beam_copy
        assert data_handler.last_selected_direct_beam_row == 0
        data_handler.set_active_cross_section.assert_called_once_with(0)
