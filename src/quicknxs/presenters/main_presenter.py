...
from quicknxs.views.widgets import AcceptRejectDialog, ActiveDataRadioButton, BinningTypeSelection
...
class MainPresenter:
    ...
    def __init__(self, main_window):
        ...
        self._data_presenter: DataPresenter = main_window.data_presenter
        ...
        self._path_watcher = QtCore.QFileSystemWatcher([self._data_presenter.current_directory], self.main_window)
    ...
    def empty_cache(self):
        ...
        self._data_presenter.clear_cache()
    ...
    def open_file(self, file_path: str, force: bool = False, silent: bool = False) -> None:
        ...
        # 2. Invoke DataPresenter.load()
        # 3. if silent==False, invoke DataPresenter.file_loaded()
        ...
        try:
            ...
            self._data_presenter.load(file_path, configuration, force=force, progress=prog)
            self.report_message(f"Loaded file(s) {self._data_presenter.current_file_name}")
        except (RuntimeError, CrossSectionError) as run_err:
            if isinstance(run_err, RuntimeError):
                self.report_message(
                    f"Error loading file(s) {self._data_presenter.current_file_name} due to:\n{run_err}",
    ...
    def file_loaded(self):
        ...
        self.main_window.auto_change_active = True
        self._set_data_presenter_active_cross_section()
    ...
    def _set_data_presenter_active_cross_section(self):
        ...
        success = self._data_presenter.set_active_cross_section(current_cross_section)
        if not success:
    ...
    def update_tables(self):
        ...
        idx = self._data_presenter.find_active_data_id()
        if idx is not None:
        ...
        idx = self._data_presenter.find_active_direct_beam_id()
        if idx is not None:
            self.update_direct_beam_table(idx, self._data_presenter.active_cross_section)
            direct_beam = self._data_presenter.direct_beam_list[idx]
    ...
    def update_file_list(self, query_path: Optional[str] = None) -> None:
        ...
        def _updated_current_list():
            ...
            return sorted(self._data_presenter.current_event_files + composites)
        ...
        def _reset_ui_file_list(fresh_list):
            ...
            for item in fresh_list:
                ...
                if item == self._data_presenter.current_file_name:
                ...
                if item in self._data_presenter.bad_files:
        ...
        def _update_current_directory(new_dir):
            ...
            self._path_watcher.removePath(self._data_presenter.current_directory)
            self._data_presenter.current_directory = new_dir
            self._path_watcher.addPath(self._data_presenter.current_directory)
        ...
        # Use case 2: a composite from using Open Sum
            ...
            if file_dir == self._data_presenter.current_directory:
            ...
            # Use case 2.2: the composite is made up of files in a new directory
                ...
                new_list = sorted(self._data_presenter.current_event_files + [file_path.basename])
        # Use case 3: a single path pointing to a file or a directory
        else:
            # Use case 3.1: a single path pointing to a new directory
            if os.path.isdir(query_path):
                file_dir = query_path
                if file_dir != self._data_presenter.current_directory:  # User changed directory
                    _update_current_directory(file_dir)
                    new_list = self._data_presenter.current_event_files
                else:
            ...
            # Use case 3.2: a single path pointing to a file in the current or new directory
                ...
                file_dir, _ = file_path.split()
                if file_dir == self._data_presenter.current_directory:  # User selected a new file in the directory
                    new_list = _updated_current_list()
    ...
    def automated_file_selection(self):
        ...
        event_file_list = glob.glob(os.path.join(self._data_presenter.current_directory, "*event.nxs"))
        h5_file_list = glob.glob(os.path.join(self._data_presenter.current_directory, "*.nxs.h5"))
        ...
        n_count = 0
        logging.error("Current file: %s", self._data_presenter.current_file_name)

        q_current = self._data_presenter.extract_metadata().mid_q

        # Add the current data set to the reduction table
        # Do nothing if the data is incompatible
        is_direct_beam = self._data_presenter.active_cross_section.is_direct_beam
        if is_direct_beam:
        ...
        for f in event_file_list:
            file_path = str(os.path.join(self._data_presenter.current_directory, f))
            if current_file_found and n_count < 10:
                n_count += 1
                metadata = self._data_presenter.extract_metadata(file_path)
                ...
                if q_current <= metadata.mid_q and is_direct_beam == metadata.is_direct_beam:
                    ...
                    if not is_direct_beam == self._data_presenter.active_cross_section.is_direct_beam:
                        break
                    self.main_window.auto_change_active = True
                    self.populate_from_configuration(d.configuration)
                    if self._data_presenter.active_cross_section.is_direct_beam:
                        self.add_direct_beam()
                    else:
                        self.add_reflectivity()

            if f == self._data_presenter.current_file_name:
    ...
    def open_reduced_file_dialog(self):
        ...
        if file_path:
            ...
            try:
                self._data_presenter.load_data_from_reduced_file(file_path, configuration=configuration, progress=prog)
            ...
            self.ui.directBeamTable.setRowCount(len(self._data_presenter.direct_beam_list))
            for idx, _ in enumerate(self._data_presenter.direct_beam_list):
                self._data_presenter.set_active_data_from_direct_beam_list(idx)
                self.update_direct_beam_table(idx, self._data_presenter.active_cross_section)
            # Update UI data table(s) with the loaded data
            for ipeak, _ in self._data_presenter.peak_reduction_lists.items():
                self._data_presenter.set_active_reduction_list_index(ipeak)
                self.main_window.add_data_tab_by_index(ipeak)
                table_widget = self.get_reduction_table_by_index(ipeak)
                table_widget.setRowCount(len(self._data_presenter.reduction_list))
                for idx, _ in enumerate(self._data_presenter.reduction_list):
            ...
            if self._data_presenter.active_cross_section is not None:
                self.populate_from_configuration(self._data_presenter.active_cross_section.configuration)
                self.update_file_list(self._data_presenter.current_file)
            self.main_window.auto_change_active = False
    ...
    def initialize_additional_reduction_table(self, tab_index: int):
        ...
        if self._data_presenter.main_reduction_list:
            table_widget = self.get_reduction_table_by_index(tab_index)
            table_widget.setRowCount(len(self._data_presenter.main_reduction_list))
            active_cross_section_name: str = self._data_presenter.active_cross_section.name
            for idx, nexus_data in enumerate(self._data_presenter.main_reduction_list):
    ...
    def _file_open_dialog(self, filter_: Optional[str] = None) -> Optional[str]:
        ...
        """
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self.main_window, "Open NXS file...", directory=self._data_presenter.current_directory, filter=filter_
        )
        return file_path
    ...
    def _file_open_sum_dialog(self, filter_: Optional[str] = None) -> Optional[str]:
        ...
        file_paths, _ = QtWidgets.QFileDialog.getOpenFileNames(
            ...
            directory=self._data_presenter.current_directory,
            filter=filter_,
        )
    ...
    def file_open_dialog(self):
        """GUI callback for backend MainPresenter._file_open_dialog."""
        self._process_file_path("_file_open_dialog")

    def file_open_sum_dialog(self):
        """GUI callback for backend MainPresenter._file_open_sum_dialog."""
    ...
    def update_daslog(self):
        ...
        table.setRowCount(0)
        table.sortItems(-1)
        table.setColumnCount(len(self._data_presenter.data_sets) + 2)
        table.setHorizontalHeaderLabels(["Name"] + list(self._data_presenter.data_sets.keys()) + ["Unit"])
        for j, key in enumerate(sorted(self._data_presenter.active_cross_section.logs.keys(), key=lambda s: s.lower())):
            table.insertRow(j)
            table.setItem(j, 0, QtWidgets.QTableWidgetItem(key))
            table.setItem(
                j,
                len(self._data_presenter.data_sets) + 1,
                QtWidgets.QTableWidgetItem(self._data_presenter.active_cross_section.log_units[key]),
            )
            i = 0
    ...
    def add_reflectivity(self, silent=False):
        ...
        idx = self._data_presenter.find_run_number_in_reduction_list(self._data_presenter._nexus_data)
        if idx is None:
            raise RuntimeError(
    ...
    def update_reduction_table(self, table_widget: QtWidgets.QTableWidget, idx: int, data: CrossSectionData):
        ...
        if idx < len(self._data_presenter.reduction_list):
            nexus_data = self._data_presenter.reduction_list[idx]
    ...
    def clear_reflectivity(self):
        ...
        self._data_presenter.clear_reduction_lists()
    ...
    def remove_reflectivity(self):
        ...
        if index < 0:
            return
        self._data_presenter.reduction_list.pop(index)
    ...
    def reduction_table_changed(self, item: QtWidgets.QTableWidgetItem):
        ...
        refl = self._data_presenter.reduction_list[row]
        ...
        match column:
            ...
            case ReductionTableColumn.DIRECT_BEAM:
                ...
                direct_beam = self._data_presenter.find_direct_beam_by_name(direct_beam_name)
                if direct_beam:
    ...
    def reduction_table_cell_changed(self, refl: NexusData, recalculate: bool = True):
        ...
        if self._data_presenter.is_active(refl):
            self.main_window.auto_change_active = True
            ...
            table_widget = self.reduction_table
            idx = self._data_presenter.find_run_number_in_reduction_list(refl)
            active_cross_section_name: str = self._data_presenter.active_cross_section.name
            active_cross_section = refl.cross_sections[active_cross_section_name]
            self.update_reduction_table(table_widget, idx, active_cross_section)

        # Update the direct beam table if this data set is in it
        idx = self._data_presenter.find_run_number_in_direct_beam_list(refl)
        if idx is not None:
    ...
    def update_direct_beam_table(self, idx: int, data: CrossSectionData) -> None:
        ...
        radio_widget = ActiveDataRadioButton(
            self, is_active=(data == self._data_presenter.active_cross_section), idx=idx, is_direct_beam=True
        )
    ...
    def direct_beam_table_changed(self, item: QtWidgets.QTableWidgetItem):
        ...
        data = self._data_presenter.direct_beam_list[item.row()]
        ...
        try:
            ...
            old_value = getattr(self._data_presenter.active_cross_section.configuration, keys[col])
        ...
        if self._data_presenter.is_active(data):
            self.main_window.auto_change_active = True
            self.update_overview_run_info_from_active_run()
            self.main_window.auto_change_active = False
    ...
    def active_data_changed(self):
        ...
        current_file_name = self._data_presenter.current_file_name
        for i in range(self.ui.file_list.count()):
    ...
    def reduction_table_right_click(self, pos: QtCore.QPoint, is_reduction_table: bool = True):
        ...
        def _propagate_run(_pos):
            ...
            if 0 <= row < len(data_table):
                nexus_data = data_table[row]
                active_cross_section = self._data_presenter.active_cross_section.name
                active_cross_section = nexus_data.cross_sections[active_cross_section]
                for ipeak, peak_data in self._data_presenter.peak_reduction_lists.items():
        ...
        if is_reduction_table:
            table_widget = self.reduction_table
            data_table = self._data_presenter.reduction_list
        else:
            table_widget = self.ui.directBeamTable
            data_table = self._data_presenter.direct_beam_list
    ...
    def compute_offspec_on_change(self, force=False):
        ...
        offspec_data_exists = self._data_presenter.is_offspec_available()
        ...
        if force or has_changed_values >= 0 or not offspec_data_exists:
            ...
            config = self.get_configuration_from_ui()
            self._data_presenter.update_configuration(configuration=config, active_only=False)
            self._data_presenter.reduce_offspec(progress=prog)

    def compute_gisans_on_change(self, force=False, active_only=True):
        """Compute GISANS as needed."""
        prog = self.new_progress_reporter()
        has_changed_values = self.check_region_values_changed()
        gisans_data_exists = self._data_presenter.is_gisans_available(active_only=active_only)
        ...
        if force or has_changed_values >= 0 or not gisans_data_exists:
            ...
            self._data_presenter.update_configuration(configuration=config, active_only=False)
            if active_only:
                result = self._data_presenter.calculate_gisans(progress=prog)
                if not result:
                    self.report_message(
                        f"Could not compute GISANS for {self._data_presenter.current_file_name}",
                        detailed_message=str(traceback.format_exc()),
                        pop_up=True,
                        is_error=True,
                    )
            else:
                self._data_presenter.reduce_gisans(progress=prog)
    ...
    def check_region_values_changed(self):
        ...
        configuration = self._data_presenter.active_cross_section.configuration
        valid_change = False
        replot_change = False
    ...
    def stitch_reflectivity(self):
        ...
        try:
            self._data_presenter.stitch_data_sets(
    ...
    def trim_data_to_normalization(self):
        ...
        trim_points = self._data_presenter.get_trim_values()
    ...
    def strip_overlap(self):
        ...
        self._data_presenter.strip_overlap()
    ...
    def reload_all_files(self):
        ...
        """
        if self._data_presenter.get_cachesize() == 0:
            return

        # Store the active (plotted) run index
        active_idx = self._data_presenter.find_active_data_id()
        if active_idx is not None:
            is_active_data_direct_beam = False
        else:
            is_active_data_direct_beam = True
            active_idx = self._data_presenter.find_active_direct_beam_id()

        # Store the active data tab
        active_data_tab = self._data_presenter.active_reduction_list_index

        # Reload files
        self._data_presenter.clear_cached_unused_data()
        configuration = self.get_configuration_from_ui()
        prog = ProgressReporter(progress_bar=self.progress_bar, status_bar=self.status_bar_handler)
        try:
            self._data_presenter.reload_files(configuration, prog)
        except CrossSectionError as err:
            self._show_diagnostic(err)
            return
        ...
        for idx, _ in enumerate(self._data_presenter.direct_beam_list):
            self._data_presenter.set_active_data_from_direct_beam_list(idx)
            self.update_direct_beam_table(idx, self._data_presenter.active_cross_section)
        self.ui.reductionTable.setRowCount(len(self._data_presenter.reduction_list))
        for ipeak, peak_data in self._data_presenter.peak_reduction_lists.items():
            self._data_presenter.set_active_reduction_list_index(ipeak)
            table_widget = self.get_reduction_table_by_index(ipeak)
            table_widget.setRowCount(len(self._data_presenter.reduction_list))
        ...
        self._data_presenter.set_active_reduction_list_index(active_data_tab)

        # Restore the active run
        if is_active_data_direct_beam:
            self._data_presenter.set_active_data_from_direct_beam_list(active_idx)
        else:
            self._data_presenter.set_active_data_from_reduction_list(active_idx)
