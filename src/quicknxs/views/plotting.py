...
class PlotView(object):
    ...
    def plot_overview(self):
        ...
        data: CrossSectionData = main_window.data_presenter.active_cross_section
    ...
    def plot_xy(self):
        ...
        main_window = self.main_window
        data_set_keys = list(main_window.data_presenter.data_sets.keys())
        ...
        for i, key in enumerate(data_set_keys[:4]):
            dataset = main_window.data_presenter.data_sets[key]
        ...
        for i, datai in enumerate(xynormed):
            if main_window.data_presenter.data_sets[data_set_keys[i]].total_counts == 0:
                continue
            if main_window.ui.tthPhi.isChecked():
                plots[i].clear()
    ...
    def plot_xtof(self):
        ...
        wavelength = main_window.data_presenter.active_cross_section.wavelength
        tof = main_window.data_presenter.active_cross_section.tof
    ...
    def plot_projections(self, preserve_lim=False):
        ...
        """
        main_window = self.main_window
        if main_window.data_presenter.active_cross_section is None:
            return
        data = main_window.data_presenter.active_cross_section
        data.prepare_plot_data()
    ...
    def plot_offspec(self, recalc=True, crop=False):
        ...
        """
        if self.main_window.data_presenter.active_cross_section is None:
            return
        ...
        progress = self.main_window.file_handler.new_progress_reporter()
        n_total = len(self.main_window.data_presenter.reduction_list)
        if n_total == 0:
        ...
        for i_run, nexus_data in enumerate(self.main_window.data_presenter.reduction_list):
    ...
    def _prepare_intensity_plot(self):
        ...
        counts_normalized = self.main_window.data_presenter.active_cross_section.get_tof_counts_table()[0][:, 2]
        counts_normalized_error = self.main_window.data_presenter.active_cross_section.get_tof_counts_table()[0][:, 3]

        if self.main_window.ui.xLamda.isChecked():
            self.main_window.ui.refl.errorbar(
                self.main_window.data_presenter.active_cross_section.wavelength,
                counts_normalized,
            ...
            tof_ms = self.main_window.data_presenter.active_cross_section.get_tof_counts_table()[0][:, 0] / 1000
    ...
    def plot_reflectivity_or_intensity(self):
        ...
        If the active run is a direct beam run (natively labeled as direct beam OR selected
        from the direct beam table), the intensity vs ToF (or wavelength) in the
        region-of-interest (ROI) is plotted.

        If the active run is selected from the reduction/data table, the reflectivity of all
        datasets is plotted, even if that same run also exists in the direct beam table.

        This allows users to compare different ROIs by adding the same run to both tables,
        with the plot type determined by which table the user is currently viewing from.
        ...
        if self.main_window.data_presenter.active_cross_section.is_direct_beam:
    ...
    def _prepare_reflectivity_plot(self):
        ...
        """
        if (
            self.main_window.data_presenter.active_cross_section.r is None
            or self.main_window.data_presenter.active_cross_section.q is None
            or self.main_window.data_presenter.active_cross_section.dr is None
        ):
            self._plot_message(self.main_window.ui.refl, "No data")
            return False

        data = self.main_window.data_presenter.active_cross_section
        if data.total_counts == 0:
        ...
        PN = len(self.main_window.data_presenter.active_cross_section.q) - self.main_window.ui.rangeEnd.value()
        ynormed = self.main_window.data_presenter.active_cross_section.r[P0:PN]
        ...
        self.main_window.ui.refl.errorbar(
            self.main_window.data_presenter.active_cross_section.q[P0:PN],
            ynormed,
            yerr=self.main_window.data_presenter.active_cross_section.dr[P0:PN],
            label="Active",
            lw=2,
            capsize=1,
            color="black",
        ...
        xs_name = self.main_window.data_presenter.active_cross_section.name
        for i, refli in enumerate(self.main_window.data_presenter.reduction_list):
    ...
    def plot_gisans(self):
        ...
        for i, xs in enumerate(data_set_keys):
            plot = plots[i]
            plot.show()
            selected_data = self.main_window.data_presenter.data_sets[xs]
            plots[i].clear_fig()
            plots[i].pcolormesh(
